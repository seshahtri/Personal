...
SELECT
	...
	SUM(ili.GrossFeeAmountForRates * ISNULL(ex.ExchangeRate, 1)) as ''Total Fees''
FROM V_InvoiceTimekeeperSummary ili
	...
	INNER JOIN (
		...
		FROM fn_GetRollupPracticeAreasWithSecurity (''-1'', ''|'',-1,0)
	...
	INNER JOIN (
		...
		FROM fn_GetRollupBusinessUnitsWithSecurity2 (''-1'', ''|'',-1,0)
