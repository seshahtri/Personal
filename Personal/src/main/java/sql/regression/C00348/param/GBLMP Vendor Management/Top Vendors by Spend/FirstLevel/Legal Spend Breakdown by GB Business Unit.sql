...
exec as user = ''admin''
select 
TOP 1 b.RollupBusinessUnitId,
b.RollupBusinessUnitName as [Business Unit Name], ed.currencycode,sum(f.amount) as ''Total_Spend'',
sum(f.amount)/(overall.Overallspend) as ''% Of Total Spend'',sum(f.NetFeeAmount) as Fees,sum(f.NetExpAmount) as Expenses
...
FROM [dbo].[fn_GetRollupBusinessUnitsWithSecurity] (-1, ''|'',-1,1)) b on bu.BusinessUnitId=b.RollupBusinessUnitId --replace 0 with 1 and apply BU filter
...
--AND VendorID = 70237
AND VendorID = ^ParamOne^
group by b.RollupBusinessUnitId,b.RollupBusinessUnitName, ed.currencycode,overall.Overallspend
