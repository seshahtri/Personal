  ...
  JOIN businessunitandalldescendants BUD 
       ...
       JOIN 
         ...
         JOIN practiceareaandalldescendants PAD 
               ...
               JOIN (SELECT rolluppracticeareaid, 
                     ...
                     FROM   Fn_getrolluppracticeareaswithsecurity (''-1'', ''|'', ^ParamTwo^, 1)) PASEC
					 --FROM   Fn_getrolluppracticeareaswithsecurity (''-1'', ''|'', ''44766'', 1)) PASEC 
	...
	LEFT JOIN (
			...
			WHERE
				...
				AND vb.VendorId = ^ParamOne^
				--AND vb.VendorId = 44228 --Vendor:Wilson, Elser, Moskowitz, Edelman & Dicker, LLP---New York|
				GROUP BY
				ili.BudgetPeriodId,
				ili.MatterId,
				ili.VendorId
