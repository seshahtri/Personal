...
from (SELECT 
    ...
    IsNull(Spend/NULLIF(SUM(BudgetAmount),0) *100,0) [Percent of Budget consumed], 
    CASE 
        WHEN 
    ...
    SUM(BudgetAmount) - ISNull(Spend,0) [Budget Remaining]
FROM 
    (    SELECT 
...
JOIN 
    ...
    FROM 
        V_VendorBudgetInfo VBI	
		INNER JOIN [dbo].[BusinessUnitAndAllDescendants] [BusinessUnitAndAllDescendants] ON (VBI.[BusinessUnitId] = [BusinessUnitAndAllDescendants].[ChildBusinessUnitId])
        INNER JOIN (
        ...
        FROM [dbo].[fn_GetRollupBusinessUnitsWithSecurity] (''-1'', ''|'',^ParamTwo^,1)
		--FROM [dbo].[fn_GetRollupBusinessUnitsWithSecurity] (''-1'', ''|'',16666,1)
    ...
    GROUP BY 
        ...
        VBI.MatterDF07,VBI.MatterDF08,VBI.MatterDF09,VBI.CurrencyCode,VBI.MatterOwnerId/*Anand*/  ) Spend ON Budget.MatterId = Spend.MatterId
GROUP BY 
...
ORDER BY Spend DESC,[Claim Number] DESC'
