--Base Level: Legal Spend Breakdown by GB Business Unit
...
SET @SQL='
	...
	INNER JOIN 
			...
			FROM fn_GetRollupBusinessUnitsWithSecurity (''-1'', ''|'',-1,1) 
	...
	INNER JOIN 
			(SELECT Sum(invs.Amount*ISNULL(ed.ExchangeRate,1)) OverallSpend         
			FROM V_InvoiceSummary invs             
			join ExchangeRateDim ed on ed.ExchangeRateDate = invs.ExchangeRateDate             
			INNER JOIN BusinessUnitAndAllDescendants bud ON invs.BusinessUnitId = bud.ChildBusinessUnitId             
