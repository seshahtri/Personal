...
isnull( (1- ( SUM((CASE WHEN (NOT (InvoiceStatus.Name IS NULL))  THEN (InvoiceSummary.Amount * ExchangeRateDim.ExchangeRate) ELSE CAST(NULL AS FLOAT)  END)) )/   (MIN((V_MatterBudgetDetails.BudgetAmount * ExchangeRateDim1.ExchangeRate)))),100) * 100 as [Percent of Budget Remaining],
...
FROM dbo.V_MatterBudgetDetails V_MatterBudgetDetails
  ...
  -- FROM [dbo].[fn_GetRollupBusinessUnitsWithSecurity] (''16548'',''|'',''16558'',0)
   FROM [dbo].[fn_GetRollupBusinessUnitsWithSecurity] (^ParamOne^, ''|'',^ParamTwo^,0)
...
GROUP BY V_MatterBudgetDetails.MatterDF07,
  ...
  order by [Total Budget] desc,Spend desc,V_MatterBudgetDetails.MatterName asc'
  Print (@SQL)
EXEC(@SQL)