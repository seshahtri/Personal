...
SET @SQL=' 
select       
	  bu.RollupBusinessUnitName, 
	  ed.CurrencyCode as ''Currency Code'',       
	  Sum(invs.GrossFeeAmountForRates*ISNULL(ed.ExchangeRate,1)) as ''Total Spend'',       
	  Sum(invs.GrossFeeAmountForRates*ISNULL(ed.ExchangeRate,1))/overall.OverallSpend * 100 as ''% Of Total'',       
	  Sum(invs.NetFeeAmountForRates*ISNULL(ed.ExchangeRate,1)) as ''Fees'',      
...
from V_InvoiceTimekeeperSummary invs      
	  ...
	  INNER JOIN (SELECT 1 BUCheck, RollupBusinessUnitId, RollupBusinessUnitName, RollupBusinessUnitPath, RollupBusinessUnitDisplayPath, RollupBusinessUnitLevel
                  FROM fn_GetRollupBusinessUnitsWithSecurity (-1, ''|'',-1,1)) bu ON bud.BusinessUnitId = bu.RollupBusinessUnitId      
	  ...
	  INNER JOIN (SELECT RollupPracticeAreaId, RollupPracticeAreaName, RollupPracticeAreaPath, RollupPracticeAreaDisplayPath, RollupPracticeAreaLevel          
				  FROM [dbo].[fn_GetRollupPracticeAreasWithSecurity2] (-1, ''|'',^ParamOne^,0)) pa on pa.RollupPracticeAreaId=p.PracticeAreaId      
	  INNER JOIN (SELECT Sum(invs.GrossFeeAmountForRates*ISNULL(ed.ExchangeRate,1)) OverallSpend 
