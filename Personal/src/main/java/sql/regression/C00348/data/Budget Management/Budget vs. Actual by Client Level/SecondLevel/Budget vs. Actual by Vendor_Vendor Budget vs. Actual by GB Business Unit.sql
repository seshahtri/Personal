  ...
  JOIN businessunitandalldescendants BUD 
       ...
       JOIN 
         ...
         JOIN practiceareaandalldescendants PAD 
               ...
               JOIN (SELECT rolluppracticeareaid, 
                     ...
                     FROM   Fn_getrolluppracticeareaswithsecurity (^ParamOne^, ''|'', -1 , 1)) 
					 --FROM   Fn_getrolluppracticeareaswithsecurity (''42406'', ''|'', -1 , 1)) 
                    PASEC 
	...
	LEFT JOIN (
			...
			WHERE
				...
				AND vb.VendorId = ^ParamTwo^
				--AND vb.VendorId = 72861
				GROUP BY
				ili.BudgetPeriodId,
				ili.MatterId,
				ili.VendorId
...
THEN (CASE WHEN (((vb.[MatterOpenDate] <= {d ''2021-12-31''}) 
