...
FROM V_InvoiceSummary ili   
	...
	JOIN (SELECT YEAR (ili.PaymentDate) as ''Year'',     
		  ...
		  GROUP BY YEAR (ili.PaymentDate), MONTH(ili.PaymentDate)) as Amt on Amt.Year=YEAR (ili.PaymentDate)  and Amt.Month=MONTH(ili.PaymentDate)  
WHERE 
	ili.InvoiceStatus IN (''' + @InvoiceStatus + ''')    
	AND  (ili.PaymentDate >= ''' + @pDateStart + ''' AND ili.PaymentDate<= ''' + @pDateEnd + ''')  
	AND ((ex.Currencycode = ''' + @CurrencyCode + ''') OR (ex.CurrencyId IS NULL))    
	AND MatterId = ^ParamTwo^
	GROUP BY YEAR (ili.PaymentDate), DateName(month, DateAdd(month, MONTH(ili.PaymentDate), -1)), MONTH(ili.PaymentDate), Amt.Spend, Amt.Hours  
