...
FROM
    ...
    JOIN
            ...
            WHERE
				...
				--AND ed.Currencycode = ''USD''
				AND ed.CurrencyCode = ''' + @CurrencyCode + '''
				AND (its.PaymentDate >= ''' + @pDateStart + ''' AND its.PaymentDate<= ''' + @pDateEnd + ''')
                AND its.TimekeeperRoleId is NOT NULL
                AND its.HOURS > 0.01
                AND its.RoleName is NOT NULL
				
                and grossfeeamountforrates>0
                and its.VendorId=^ParamOne^
				and its.MatterId = ^ParamTwo^
				 --and its.VendorId=73219
				 --and its.MatterId =18501379
