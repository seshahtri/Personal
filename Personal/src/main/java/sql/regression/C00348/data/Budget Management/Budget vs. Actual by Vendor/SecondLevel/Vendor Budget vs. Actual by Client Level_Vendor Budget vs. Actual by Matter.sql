...
from (SELECT 
    ...
    [Spend], 
    ISNull(SUM(BudgetAmount),0) TotalBudget, 
...
JOIN 
    ...
    FROM 
        ...
        JOIN (SELECT RollupPracticeAreaId, RollupPracticeAreaName, RollupPracticeAreaPath, RollupPracticeAreaDisplayPath, RollupPracticeAreaLevel
                FROM fn_GetRollupPracticeAreasWithSecurity (''-1'', ''|'',^ParamTwo^,0)) PASEC ON PAD.PracticeAreaId = PASEC.RollupPracticeAreaId
				--FROM fn_GetRollupPracticeAreasWithSecurity (''-1'', ''|'',44766,0)) PASEC ON PAD.PracticeAreaId = PASEC.RollupPracticeAreaId
