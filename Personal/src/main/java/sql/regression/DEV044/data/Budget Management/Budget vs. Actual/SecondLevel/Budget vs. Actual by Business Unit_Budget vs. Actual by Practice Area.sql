...
SET @SQL='
exec as user=''admin''
SELECT [RollupPracticeAreasWithSecurity].[RollupPracticeAreaName] AS [Practice Area], 
...
ELSE 0 END),4) as [Percent of Budget Remaining], 
round(cast((sum(CASE ISNULL([InvoiceSummary].[budgetrnk], 1) WHEN 1 then 
([V_MatterBudgetDetails].[BudgetAmount]*[ExchangeRateDim1].[ExchangeRate])  ELSE 0 end)-
sum(CASE WHEN (NOT ([Invoice Status].[Name] IS NULL)) then [InvoiceSummary].[amount] * [ExchangeRateDim].[ExchangeRate]
ELSE CAST(NULL AS FLOAT) end )) as float),7) AS [Budget Remaining]
...
--FROM [dbo].[fn_GetRollupBusinessUnitsWithSecurity] (''-1'',''|'',14883,0)
FROM [dbo].[fn_GetRollupBusinessUnitsWithSecurity] (''-1'',''|'',^ParamTwo^,0)
...
--and V_MatterBudgetDetails.BudgetPeriodId = 3
and V_MatterBudgetDetails.BudgetPeriodId = ^ParamOne^
GROUP BY
