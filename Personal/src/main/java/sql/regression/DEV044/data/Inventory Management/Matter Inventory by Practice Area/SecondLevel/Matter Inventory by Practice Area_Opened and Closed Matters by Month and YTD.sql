...
IF OBJECT_ID(''tempdb..#MatterStatusHistory'') IS NOT NULL DROP TABLE #MatterStatusHistory
  ...
  SELECT MatterId, ''Open'' MatterHistoryStatus, MatterOpenDate StartDate, ''3000-01-01 00:00:00.000'' EndDate,PracticeAreaId
  ...
  SELECT MatterId, ''Open'', MatterOpenDate StartDate, MatterCloseDate EndDate,PracticeAreaId
  from MatterDim M
  WHERE MatterStatus = ''Closed''
  
  UNION ALL
  
  SELECT MatterId, ''Closed'', MatterCloseDate  StartDate, ''3000-01-01 00:00:00.000'' EndDate,PracticeAreaId
  from MatterDim M
  WHERE MatterStatus = ''Closed''
...
DATEPART(Month,(CASE WHEN (mh.StartDate >= '''+@pDateStart+''') THEN mh.StartDate WHEN NOT (mh.StartDate >= '''+@pDateStart+''')
THEN '''+@pDateStart+''' ELSE NULL END)) AS MHMonth
...
) pa1 ON   ( mc.PracticeArea = pa1.RollupPracticeAreaId) -- Comment/Uncomment to filter based on Practice Area Name--

ORDER BY 
pa1.RollupPracticeAreaName,  -- Comment/Uncomment to filter based on Practice Area Name--
