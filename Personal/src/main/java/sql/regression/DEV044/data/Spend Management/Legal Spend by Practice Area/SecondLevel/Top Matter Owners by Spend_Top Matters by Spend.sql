...
SET @SQL = '
exec as user=''admin''
Select  f.MatterName as ''Matter Name'', p.PracticeAreaName as ''Practice Area'', f.MatterStatus as ''Matter Status'', E.currencyCode as ''Currency Code'' ,
--LTRIM(STR(CAST(sum(il.Amount) AS FLOAT), 25, 5)) [Spend]
CAST(sum(il.Amount) AS FLOAT) [Spend]
 from matterdim f 
 LEFT JOIN TimekeeperDim d ON f.MatterOwnerId = d.TimekeeperId
 join PracticeAreaDim p on f.PracticeAreaId=p.PracticeAreaId
 ...
 join (SELECT RollupPracticeAreaId, RollupPracticeAreaName
	FROM [dbo].[fn_GetPracticeAreasWithChildrenWithRollupPracticeArea] (''-1'', ''|'',^ParamOne^)) pa on pa.RollupPracticeAreaId=f.PracticeAreaId
	--FROM [dbo].[fn_GetPracticeAreasWithChildrenWithRollupPracticeArea] (''-1'', ''|'',32475)) pa on pa.RollupPracticeAreaId=f.PracticeAreaId
			
 where f.MatterOwnerId = ^ParamTwo^
 --f.MatterOwnerId = 742382
