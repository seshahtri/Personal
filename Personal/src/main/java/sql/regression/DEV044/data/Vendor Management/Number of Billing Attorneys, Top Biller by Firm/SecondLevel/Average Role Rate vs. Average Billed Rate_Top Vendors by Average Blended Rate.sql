...
SET @SQL='
	SELECT      
		...
		SUM((ili.GrossFeeAmountForRates) * ISNULL(ex.ExchangeRate, 1))/SUM(CASE WHEN ili.HoursForRates=0 THEN NULL ELSE ili.HoursForRates END) as ''Blended Avg. Rate'',    
	...
	ORDER BY SUM((ili.GrossFeeAmountForRates) * ISNULL(ex.ExchangeRate, 1))/SUM(CASE WHEN ili.HoursForRates=0 THEN NULL ELSE ili.HoursForRates END) desc,      
