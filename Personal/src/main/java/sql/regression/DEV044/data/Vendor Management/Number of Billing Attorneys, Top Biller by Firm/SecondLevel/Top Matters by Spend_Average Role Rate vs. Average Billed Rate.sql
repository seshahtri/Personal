...
SELECT
	...
	JOIN (
			...
			FROM V_InvoiceLineItemSpendFactWithCurrency ili
				JOIN TimekeeperRoleDim tk on tk.TimekeeperRoleId = ili.TimekeeperRoleId
			WHERE
			--ili.matterid=3686999 and ili.vendorid=1044
			ili.matterid=^ParamTwo^ and ili.vendorid=^ParamOne^ 
...
WHERE --ili.matterid=3686999 and ili.vendorid=1044 and
ili.matterid=^ParamTwo^ and ili.vendorid=^ParamOne^ and
