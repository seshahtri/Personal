...
--FROM [dbo].[fn_GetRollupPracticeAreasWithSecurity2] (''-1'', ''|'',32470,1)) pa on pa.RollupPracticeAreaId=p.PracticeAreaId
WHERE ifbc.InvoiceStatus IN ('''+@InvoiceStatus+''')

and m.MatterId = ^ParamTwo^
--and m.MatterId = ''5595523''
...
ORDER BY PhaseCode,Year, Sum(ifbc.GrossFeeAmountForRates*ISNULL(ed.ExchangeRate,1)) desc /*,
