...
FROM matterdim t
	...
	LEFT OUTER JOIN (SELECT TOP 10 f.MatterOwnerId, SUM (il.Amount) as Total_Spend, SUM(il.units) as Total_units, SUM(il.Hours) as Total_Hours
					from matterdim  f
...
WHERE 
	 ...
	 --And i.vendorid=1044
	 And i.vendorid=^ParamOne^
GROUP BY t.MatterOwnerId, d.FullName, a.Total_Spend, CASE WHEN t.MatterCloseDate  IS NULL THEN ''Active'' Else ''Closed'' End, a.Total_Units,a.Total_Hours, e.CurrencyCode
