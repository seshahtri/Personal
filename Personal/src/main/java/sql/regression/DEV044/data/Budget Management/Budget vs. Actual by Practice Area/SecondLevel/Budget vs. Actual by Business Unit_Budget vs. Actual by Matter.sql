...
JOIN (SELECT RollupBusinessUnitId, RollupBusinessUnitName, RollupBusinessUnitPath, RollupBusinessUnitDisplayPath, RollupBusinessUnitLevel
  --FROM fn_GetRollupBusinessUnitsWithSecurity (''-1'', ''|'',14883,0)) BUS ON BAD.BusinessUnitId = BUS.RollupBusinessUnitId
  FROM fn_GetRollupBusinessUnitsWithSecurity (''-1'', ''|'',^ParamTwo^,0)) BUS ON BAD.BusinessUnitId = BUS.RollupBusinessUnitId
...
JOIN (SELECT RollupPracticeAreaId, RollupPracticeAreaName, RollupPracticeAreaPath, RollupPracticeAreaDisplayPath, RollupPracticeAreaLevel
  --FROM fn_GetRollupPracticeAreasWithSecurity2 (''-1'', ''|'',32475,0)) PAS ON PAD.PracticeAreaId = PAS.
  FROM fn_GetRollupPracticeAreasWithSecurity2 (''-1'', ''|'',^ParamOne^,0)) PAS ON PAD.PracticeAreaId = PAS.RollupPracticeAreaId
