...
SELECT 
 ...
 (SUM((CASE ISNULL(INV.budgetrnk, 1) WHEN 1 THEN (MB.BudgetAmount * ER1.ExchangeRate) ELSE 0 END))  - SUM((CASE WHEN (NOT (INVS.[Name] IS NULL)) THEN (INV.amount * ER.ExchangeRate) ELSE NULL END)) 
 )    AS [Budget Remaining] 
...
JOIN (SELECT RollupBusinessUnitId, RollupBusinessUnitName, RollupBusinessUnitPath, RollupBusinessUnitDisplayPath, RollupBusinessUnitLevel
  FROM fn_GetRollupBusinessUnitsWithSecurity (''-1'', ''|'',-1,1)) BUS ON BAD.BusinessUnitId = BUS.RollupBusinessUnitId
LEFT JOIN (
...
JOIN (SELECT RollupPracticeAreaId, RollupPracticeAreaName, RollupPracticeAreaPath, RollupPracticeAreaDisplayPath, RollupPracticeAreaLevel
  --FROM fn_GetRollupPracticeAreasWithSecurity2 (''32504'', ''|'',32475,0)) PAS ON PAD.PracticeAreaId = PAS.RollupPracticeAreaId
  FROM fn_GetRollupPracticeAreasWithSecurity2 (''^ParamTwo^'', ''|'',^ParamOne^,0)) PAS ON PAD.PracticeAreaId = PAS.RollupPracticeAreaId
