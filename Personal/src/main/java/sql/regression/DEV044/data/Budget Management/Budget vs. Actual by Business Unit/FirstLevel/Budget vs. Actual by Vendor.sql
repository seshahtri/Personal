...
CASE WHEN CAST(1-((SUM(amt.Amount)/SUM(vb.BudgetAmount))) AS FLOAT) *100 >0 THEN CAST(1-((SUM(amt.Amount)/SUM(vb.BudgetAmount))) AS FLOAT)* 100
ELSE 100 END as [Percent of Budget Remaining],
