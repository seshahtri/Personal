...
from 
  (
    ...
    from 
	 ...
	 inner join (SELECT RollupPracticeAreaId, RollupPracticeAreaName
FROM [dbo].[fn_GetPracticeAreasWithChildrenWithRollupPracticeArea] (''-1'', ''|'',^ParamOne^)) pa on pa.RollupPracticeAreaId=[V_InvoiceLineItemSpendFactWithCurrency].PracticeAreaId
--FROM [dbo].[fn_GetPracticeAreasWithChildrenWithRollupPracticeArea] (''-1'', ''|'',32475)) pa on pa.RollupPracticeAreaId=[V_InvoiceLineItemSpendFactWithCurrency].PracticeAreaId
    WHERE 
      (
        ([V_InvoiceLineItemSpendFactWithCurrency].[InvoiceStatus] in ('''+@InvoiceStatus+'''))
		and [V_InvoiceLineItemSpendFactWithCurrency].[MatterID]=^ParamTwo^
