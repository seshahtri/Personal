...
FROM matterdim t
	...
	FROM [dbo].[fn_GetPracticeAreasWithChildrenWithRollupPracticeArea] (''^ParamTwo^'', ''|'',^ParamOne^)) pa on pa.RollupPracticeAreaId=t.PracticeAreaId
	--FROM [dbo].[fn_GetPracticeAreasWithChildrenWithRollupPracticeArea] (''32504'', ''|'',32475)) pa on pa.RollupPracticeAreaId=t.PracticeAreaId
	...
	LEFT OUTER JOIN (SELECT  f.MatterOwnerId, SUM (il.Amount) as Total_Spend, SUM(il.units) as Total_Units, SUM(il.units) as Total_Hours
			...
			join invoicelineitemfact il on  i.invoiceid=il.invoiceid
			join (SELECT RollupPracticeAreaId, RollupPracticeAreaName
	FROM [dbo].[fn_GetPracticeAreasWithChildrenWithRollupPracticeArea] (''^ParamTwo^'', ''|'',^ParamOne^)) pa on pa.RollupPracticeAreaId=f.PracticeAreaId
	--FROM [dbo].[fn_GetPracticeAreasWithChildrenWithRollupPracticeArea] (''32504'', ''|'',32475)) pa on pa.RollupPracticeAreaId=f.PracticeAreaId
