...
case when CAST(SUM(amt.Amount) AS FLOAT) >0.0 then CAST(SUM(amt.Amount)/SUM(vb.BudgetAmount) *100 AS FLOAT) 
else 0 end as [Percent of Budget Consumed],
...
case when CAST(SUM(amt.Amount) AS FLOAT) > 0.0 then CAST((SUM(vb.BudgetAmount) - SUM(amt.Amount)) AS FLOAT) 
else CAST(SUM(vb.BudgetAmount) AS FLOAT) end AS [Budget Remaining]
...
v.VendorName,
ed.CurrencyCode
ORDER BY SUM(amt.Amount) DESC, v.VendorName asc
'
Print (@SQL)
EXEC(@SQL)