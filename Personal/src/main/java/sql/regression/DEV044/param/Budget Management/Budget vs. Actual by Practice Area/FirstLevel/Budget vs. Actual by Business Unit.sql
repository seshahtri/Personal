...
SELECT 
  ...
  SUM((CASE ISNULL(INV.[budgetrnk], 1) WHEN 1 THEN (MB.BudgetAmount * ER1.ExchangeRate) ELSE 0 END)) AS [Percent of Budget Consumed],
 ...
 / NULLIF( SUM((CASE ISNULL(INV.budgetrnk, 1) WHEN 1 THEN (MB.BudgetAmount * ER1.ExchangeRate) ELSE 0 END)),0)  as [Percent of Budget Remaining],

 SUM((CASE ISNULL(INV.budgetrnk, 1) WHEN 1 THEN (MB.BudgetAmount * ER1.ExchangeRate) ELSE 0 END)) 
  -  SUM((CASE WHEN (NOT (INVS.[Name] IS NULL)) THEN (INV.amount * ER.ExchangeRate) ELSE NULL END))   AS [Budget Remaining] 
