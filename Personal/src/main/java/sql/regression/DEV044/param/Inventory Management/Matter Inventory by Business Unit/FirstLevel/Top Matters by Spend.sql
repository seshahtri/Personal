...
EXECUTE AS USER=''admin''
	...
	FROM V_InvoiceSummary ili   
		...
		INNER JOIN (SELECT 1 BUCheck, RollupBusinessUnitId, RollupBusinessUnitName, RollupBusinessUnitPath, RollupBusinessUnitDisplayPath, RollupBusinessUnitLevel    
					FROM fn_GetRollupBusinessUnitsWithSecurity (-1, ''|'',^ParamOne^,0)) bu ON bud.BusinessUnitId = bu.RollupBusinessUnitId    
					--FROM fn_GetRollupBusinessUnitsWithSecurity (-1, ''|'',14887,0)) bu ON bud.BusinessUnitId = bu.RollupBusinessUnitId
